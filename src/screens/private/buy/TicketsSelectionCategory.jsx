...
function SeatCategoryCard({
  ...
  // Filtra el pricing matrix para este asiento (por seat_category)
...
export default function TicketsSelectionCategory() {
  ...
  // ─── Cargar pricing matrix desde el seat-map ──────────────────────────────
  useEffect(() => {
    if (!showtimeId) {
    ...
    (async () => {
      try {
        ...
        // El backend devuelve `pricing.matrix` solo cuando hay una quote activa
        const matrix = seatData?.pricing?.matrix ?? [];
